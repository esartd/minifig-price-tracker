...
interface SetPersonalCollectionListProps {
  items: SetPersonalCollectionItem[];
  onItemDelete: (id: string) => void;
  onItemUpdate: (id: string, updates: Partial<SetPersonalCollectionItem>) => void;
...
export default function SetPersonalCollectionList({
  items,
  onItemDelete,
  onItemUpdate,
  showDecimals,
  ...
  staleItems = new Set(),
}: SetPersonalCollectionListProps) {
  const router = useRouter();
  const { data: session } = useSession();
  const { t } = useTranslation();
  const [editingQuantityId, setEditingQuantityId] = useState<string | null>(null);
  const [tempQuantity, setTempQuantity] = useState<string>('');
  const [moveDialogItem, setMoveDialogItem] = useState<SetPersonalCollectionItem | null>(null);
  ...
  const handleQuantityClick = (item: SetPersonalCollectionItem, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleQuantityBlur = (item: SetPersonalCollectionItem) => {
  ...
  const handleQuantityKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, item: SetPersonalCollectionItem) => {
    if (e.key === 'Enter') {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{
              ...
              <select
                ...
                onChange={async (e) => {
                  ...
                  try {
                    const response = await fetch(`/api/set-personal-collection/${item.id}/change-condition`, {
                      method: 'PATCH',
            ...
            {itemsUpdating.has(item.id) || (staleItems.has(item.id) && (!item.pricing || item.pricing.suggestedPrice === 0)) ? (
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  {t('collection.pricing.updatingPrices')}
                </div>
              </div>
            ...
            {onItemMove && (
              <button
                  onClick={async (e) => {
                    e.stopPropagation();
                    if (item.quantity === 1) {
                      setLastMovedItem({ id: item.id, minifigNo: item.box_no, condition: item.condition });
      ...
      {moveDialogItem && onItemMove && (
        <MoveDialog
          ...
          itemName={moveDialogItem.set_name}
              ...
              try {
                ...
                if (inventoryData.success) {
                  const movedItem = inventoryData.data.find((item: any) =>
                    item.box_no === lastMovedItem.minifigNo &&
