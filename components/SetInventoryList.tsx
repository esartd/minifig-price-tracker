...
import SetCardImage from './SetCard';
...
interface SetInventoryListProps {
  items: SetInventoryItem[];
  onItemDelete: (id: string) => void;
  onItemUpdate: (id: string, updates: Partial<SetInventoryItem>) => void;
...
export default function SetInventoryList({
  ...
  const handleQuantityClick = (item: SetInventoryItem, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleQuantityBlur = (item: SetInventoryItem) => {
  ...
  const handleQuantityKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, item: SetInventoryItem) => {
    if (e.key === 'Enter') {
            ...
            <div style={{
              ...
              <select
                ...
                onChange={async (e) => {
                  ...
                  try {
                    const response = await fetch(`/api/set-inventory/${item.id}/change-condition`, {
                      method: 'PATCH',
