...
export const LOCALES = ['en', 'de', 'fr', 'es', 'it', 'ja', 'nl', 'pl', 'pt', 'sv'] as const;
...
export type ThemedKey = 'Star Wars' | 'Super Heroes' | 'Harry Potter' | 'NINJAGO' | 'City' | 'Castle';

interface Template {
  /** `${name}` + this + `${theme}` -- e.g. " from the " -> "Han Solo from the Star Wars theme". */
  opening: (name: string, theme: string) => string;
  outfit: string;
  accessory: string;
  variant: (theme: string) => string;
  themed: Record<ThemedKey, string>;
}

export const TEMPLATES: Record<DescriptionLocale, Template> = {
  en: {
 ...
 */
export function buildDescriptions(name: string, theme: string): Record<string, string> {
  const hasOutfit = COLOR.test(name) && OUTFIT.test(name);
  const hasAccessory = ACCESSORY.test(name);
