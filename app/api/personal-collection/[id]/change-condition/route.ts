...
export async function PATCH(
  ...
  try {
    ...
    const currentItem = await prisma.personalCollectionItem.findUnique({
    ...
    const existingTargetItem = await prisma.personalCollectionItem.findUnique({
      where: {
    ...
    if (existingTargetItem) {
      ...
      const mergedItem = await prisma.personalCollectionItem.update({
      ...
      const updatedItem = await prisma.personalCollectionItem.update({
        where: { id: currentItem.id },
        data: {
          condition: newCondition,
          pricing_six_month_avg: pricing.sixMonthAverage,
