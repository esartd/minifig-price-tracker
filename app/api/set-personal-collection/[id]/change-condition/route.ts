...
export async function PATCH(
  ...
  try {
    ...
    const currentItem = await prisma.setPersonalCollectionItem.findUnique({
    ...
    // Check if user already has this set in the target condition
    const existingTargetItem = await prisma.setPersonalCollectionItem.findUnique({
      where: {
        userId_box_no_condition: {
          userId: session.user.id,
          box_no: currentItem.box_no,
    ...
    if (existingTargetItem) {
      ...
      const mergedItem = await prisma.setPersonalCollectionItem.update({
      ...
      const updatedItem = await prisma.setPersonalCollectionItem.update({
        where: { id: currentItem.id },
        data: {
          condition: newCondition,
          pricing_six_month_avg: pricing.sixMonthAverage,
    ...
    console.error('Error changing set collection condition:', error);
    return NextResponse.json(
