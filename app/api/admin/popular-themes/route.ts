...
export async function GET() {
  ...
  try {
    ...
    const collectionItems = await prismaPublic.minifigCatalog.findMany({
      where: { minifigure_no: { in: collectionMinifigs.map((item: any) => item.minifigure_no) } },
      select: { category_name: true }
    });

    const collectionThemeCounts = collectionItems.reduce((acc: Record<string, number>, item: any) => {
    ...
    const wishlistItems = await prismaPublic.minifigCatalog.findMany({
      where: { minifigure_no: { in: wishlistMinifigs.map((item: any) => item.minifigure_no) } },
      select: { category_name: true }
    });

    const wishlistThemeCounts = wishlistItems.reduce((acc: Record<string, number>, item: any) => {
      const theme = item.category_name.split('/')[0].trim();
      acc[theme] = (acc[theme] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
