...
export async function PATCH(
  ...
  try {
    ...
    const currentItem = await prisma.setInventoryItem.findUnique({
    ...
    const existingTargetItem = await prisma.setInventoryItem.findUnique({
      where: {
    ...
    if (existingTargetItem) {
      ...
      const mergedItem = await prisma.setInventoryItem.update({
      ...
      const updatedItem = await prisma.setInventoryItem.update({
        where: { id: currentItem.id },
        data: {
          condition: newCondition,
          pricing_six_month_avg: pricing.sixMonthAverage,
    ...
    console.error('Error changing set condition:', error);
    return NextResponse.json(
