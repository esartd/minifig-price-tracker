...
export default function SetsCollectionPage() {
  ...
  // Calculate stats
  const totalValue = filteredCollection.reduce((sum, item) => {
    return sum + (item.pricing?.suggestedPrice || 0) * item.quantity;
  }, 0);

  const totalItems = filteredCollection.reduce((sum, item) => sum + item.quantity, 0);
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '18px', color: '#525252' }}>Loading your collection...</div>
        </div>
      </div>
    );
  }
        ...
        <div style={{ background: 'white', borderBottom: '1px solid #e5e5e5' }}>
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '16px' }} className="responsive-padding">
            ...
            <p style={{
              ...
              {totalItems} sets • Collection value: {formatPrice(totalValue, 'USD')}
            </p>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              ...
              >
                <option value="all">All Conditions ({collection.length})</option>
                <option value="new">New ({collection.filter(i => i.condition === 'new').length})</option>
                <option value="used">Used ({collection.filter(i => i.condition === 'used').length})</option>
              </select>
              ...
              >
                ...
                <option value="price-high">Sort by Value (High to Low)</option>
                <option value="price-low">Sort by Value (Low to High)</option>
          ...
          {sortedCollection.length === 0 ? (
            <div style={{
              ...
              <div style={{ fontSize: '16px', marginBottom: '24px' }}>
                Add sets you want to keep to your personal collection
              </div>
              <Link
                href="/sets/browse"
                style={{
