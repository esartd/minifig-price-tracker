...
export default function PersonalCollectionPage() {
  ...
  useEffect(() => {
    const savedSortOrder = localStorage.getItem('collectionSortOrder');
    if (savedSortOrder) {
  ...
  const handleSortOrderChange = (newSortOrder: string) => {
    ...
    localStorage.setItem('collectionSortOrder', newSortOrder);
  };
  ...
  const loadCollection = async () => {
    try {
      ...
      if (data.success) {
        ...
        const convertedData = data.data.map((item: PersonalCollectionItem) => ({
          ...item,
        ...
        if (itemsNeedingRefresh.length > 0) {
          ...
          const fetchNextItem = async () => {
            ...
            try {
              if (process.env.NODE_ENV === 'development') console.log(`[${currentIndex}/${itemsNeedingRefresh.length}] Fetching price for ${item.minifigure_no}...`);
  ...
  const getSortedAndFilteredCollection = () => {
    ...
    switch (sortOrder) {
      ...
      case 'price-low':
        ...
        break;
      case 'name-az':
        filtered.sort((a, b) => a.minifigure_name.localeCompare(b.minifigure_name));
        break;
      case 'name-za':
        filtered.sort((a, b) => b.minifigure_name.localeCompare(a.minifigure_name));
        break;
      case 'id-low-high':
        filtered.sort((a, b) => a.minifigure_no.localeCompare(b.minifigure_no));
        break;
      case 'id-high-low':
        filtered.sort((a, b) => b.minifigure_no.localeCompare(a.minifigure_no));
        break;
      case 'year-newest':
        filtered.sort((a, b) => {
      ...
      <div className="collection-page-wrapper" style={{
        ...
        {collection.length > 0 && (
          <div style={{ marginBottom: '16px' }}>
            ...
            <div style={{
              ...
              <CollectionToggle currentType="minifigs" currentView="keep" />
        ...
        <div className="collection-list-container" style={{
          ...
          <div className="collection-list-header" style={{
            ...
            {/* First Row: Items label + All/New/Used filters + Search + Sort (desktop) */}
            {collection.length > 0 && (
