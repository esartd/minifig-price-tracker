...
// POST move item from personal collection to inventory
export async function POST(
  ...
  try {
    ...
    // Move to inventory
    const result = await database.moveToInventory(id, quantity);

    return NextResponse.json({ success: true, data: result });
  } catch (error) {
    console.error('Error moving item to inventory:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to move item to inventory' },
      { status: 500 }
    );
  }
}
